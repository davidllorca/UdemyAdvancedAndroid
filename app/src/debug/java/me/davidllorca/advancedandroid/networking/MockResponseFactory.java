...
class MockResponseFactory {
    ...
    String getMockResponse(Request request) {
        ...
        return MockResourceLoader.getResponseStrign(context, request.method(), endpointParts);
    }

    private String getEndpoint(Request request) {
        String url = request.url().toString();
        int queryParamStart = url.indexOf("?");
        return queryParamStart == -1 ? url.substring(startIndex) : url.substring(startIndex,
                queryParamStart);
    }
}
