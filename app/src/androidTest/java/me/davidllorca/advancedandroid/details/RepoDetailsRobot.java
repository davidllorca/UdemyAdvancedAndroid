...
class RepoDetailsRobot {
    ...
    RepoDetailsRobot verifyContributorsErrorVisibility(ViewMatchers.Visibility visibility) {
        onView(withId(R.id.tv_contributors_error)).check(matches(withEffectiveVisibility
                (visibility)));
        return this;
    }
    ...
    RepoDetailsRobot verifyContributorsLoadingVisibility(ViewMatchers.Visibility visibility) {
        onView(withId(R.id.contributor_loading_indicator)).check(matches(withEffectiveVisibility
                (visibility)));
        return this;
    }

    RepoDetailsRobot verifyContributorShown(String login) {
        onView(allOf(withId(R.id.tv_user_name), withText(login))).check(matches
